...
import { computeSharedSecret, deriveKeyAndIV } from '../crypto/crypto-primitives.js';
...
export function decryptKeyChain(
  ...
  const decrypted = Buffer.concat([decipher.update(encryptedKeyChain), decipher.final()]);
  ...
  return keyList.map(keyStruct => ({
    version: keyStruct[1],
    keyId: keyStruct[2],
    publicKey: Buffer.isBuffer(keyStruct[4]) ? keyStruct[4] : Buffer.from(keyStruct[4]),
    privateKey: Buffer.isBuffer(keyStruct[5]) ? keyStruct[5] : Buffer.from(keyStruct[5]),
