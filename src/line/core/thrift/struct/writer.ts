...
import type { ThriftFieldTuple, ThriftFieldValue, ThriftListValue, ThriftMapValue, ThriftType } from '../types.js';
 ...
 */
function isStructFieldTupleArray(value: ThriftFieldValue): value is ThriftFieldTuple[] {
  return Array.isArray(value) && value.every(item => Array.isArray(item) && item.length === 3);
}
 ...
 */
function writeHeader(parts: Buffer[], compactType: number, delta: number, fieldId: number): void {
  parts.push(
    delta > 0 && delta <= 15
...
function writeMap(parts: Buffer[], value: ThriftMapValue): void {
  ...
  for (const [key, item] of entries) {
    ...
    if (valueType === 11) {
      const encodedValue = typeof item === 'string' ? Buffer.from(item, 'utf-8') : (item as Buffer);
...
function writeList(parts: Buffer[], value: ThriftListValue): void {
  ...
  for (const item of items) {
    if (elementType === 11) {
      const encodedItem = typeof item === 'string'
 ...
 */
function writeFieldValue(parts: Buffer[], type: ThriftType, value: ThriftFieldValue): void {
  if (type === 3) {
 ...
 */
export function writeFieldsArray(fields: ThriftFieldTuple[], lastFieldId: { v: number }): Buffer {
  const parts: Buffer[] = [];
