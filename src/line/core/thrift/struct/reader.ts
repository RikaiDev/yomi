 ...
 */
export function readStructFields(buf: Buffer | Uint8Array, textStrings = false): Record<number, any> {
  let pos = 0;
  let lastFieldId = 0;
  let lastType = -1;
  ...
  const readString = () => {
    ...
    if (length < 0 || pos + length > buf.length) {
      throw new Error(`Invalid Length: len=${length} pos=${pos} buf=${buf.length} field=${lastFieldId} type=${lastType}`);
    }
  ...
  function readCollection(header: number) {
    ...
    if (size < 0 || size > 100000) {
      throw new Error(`Invalid Length: collection=${size} pos=${pos} field=${lastFieldId} type=${lastType}`);
    }
  ...
  function readMap() {
    ...
    if (size < 0 || size > 100000) {
      throw new Error(`Invalid Length: map=${size} pos=${pos} field=${lastFieldId} type=${lastType}`);
    }
    for (let i = 0; i < size; i++) {
      const key = readValue(keyType);
      map[Buffer.isBuffer(key) ? key.toString('utf-8') : String(key)] = readValue(valueType);
    }
    return map;
  }
