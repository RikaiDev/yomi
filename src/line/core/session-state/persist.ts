 ...
 */
export function parseStoredNumber(value: unknown, fallback: number | null): number | null {
...
export async function loadStoredSessionFields(credentialStore: any) {
  ...
  const tokenIssueTimeEpochSec = await credentialStore.get('line_token_issue_time_epoch_sec');
  const durationUntilRefreshInSec = await credentialStore.get('line_token_duration_until_refresh_sec');
  ...
  const individualRevision = await credentialStore.get('line_individual_revision');
 ...
 */
export async function persistLoginCredentials(credentialStore: any, loginResult: any): Promise<void> {
  if (!loginResult.authToken) {
    throw new Error('persistLoginCredentials: loginResult.authToken is missing — cannot persist a session without an auth token');
  }
  ...
  if (loginResult.tokenIssueTimeEpochSec != null) {
    await credentialStore.set('line_token_issue_time_epoch_sec', String(loginResult.tokenIssueTimeEpochSec));
  ...
  if (loginResult.durationUntilRefreshInSec != null) {
    await credentialStore.set('line_token_duration_until_refresh_sec', String(loginResult.durationUntilRefreshInSec));
 ...
 * @param loginResult - Login payload returned by the auth flow.
 */
export async function persistLoginCryptoMaterial(credentialStore: any, loginResult: any): Promise<void> {
 ...
 * @param loginResult - Login payload returned by the auth flow.
 */
export async function persistLoginBootstrap(credentialStore: any, loginResult: any): Promise<void> {
  const e2eeInfo = extractE2EEInfo(loginResult.e2eeInfo || loginResult.metaData || null);
  const secretKey = toPersistedKeyValue(loginResult.secretKey);
  if (!e2eeInfo?.encryptedKeyChain || !e2eeInfo?.serverPublicKey || !secretKey) {
  ...
  await credentialStore.set('line_e2ee_bootstrap', JSON.stringify({
