...
function isExpectedHttpStatus(
  ...
  return (
    statusCode === 410 &&
    bodyLength === 0 &&
    (path === LINE_APP_CONFIG.qrLongPollPath ||
      path === LINE_APP_CONFIG.pwlessLongPollPath)
  )
}
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (res.statusCode && res.statusCode >= 400) {
            const eventName = isExpectedHttpStatus(
              path,
              res.statusCode,
              body.length,
            )
            ...
            const logMethod = isExpectedHttpStatus(
              path,
              res.statusCode,
              body.length,
            )
          ...
          if (body.length > 0) {
            try {
              ...
              console.error(
                `[LINE] Decode failed on ${path}: ${error?.message || error} [${body.length}B]`,
              )
              console.error(
                `[LINE] Decode first 64B on ${path}: ${body.subarray(0, 64).toString('hex')}`,
              )
              console.error(
                `[LINE] Decode last 64B on ${path}: ${body.subarray(Math.max(0, body.length - 64)).toString('hex')}`,
              )
              reject(error)
            }
          } else {
