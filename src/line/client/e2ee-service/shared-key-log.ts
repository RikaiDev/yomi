...
export function buildSharedKeyEventLog(chatMid, shared, extra = {}) {
  const encryptedSharedKey = readSharedKeyResponseField(shared, 'encryptedSharedKey', 7);
  return {
    chat: chatMid,
    ...extractSharedKeyIdentity(shared),
    encrypted_shared_key_bytes: Buffer.isBuffer(encryptedSharedKey) || encryptedSharedKey instanceof Uint8Array
