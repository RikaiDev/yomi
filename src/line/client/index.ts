...
const LINE_AUTH_INVALIDATED_PATTERN = /V3_TOKEN_CLIENT_LOGGED_OUT|LOGGED_OUT|DIVESTED/i;
...
export function isLineAuthInvalidatedError(error: unknown): boolean {
  ...
  const exceptionMessage = data?.exception && typeof data.exception === 'object'
...
function normalizeTalkException(exception: Record<number, unknown>) {
  ...
  const code = numericCode != null
    ...
    : typeof rawCode === 'string'
      ? rawCode
      : 'UNKNOWN';
  const message = typeof exception?.[2] === 'string'
    ? exception[2]
...
export class LineClient extends EventEmitter {
  ...
  public issueV3TokenForPrimary!: (payload: Record<string, unknown>) => Promise<any>;
