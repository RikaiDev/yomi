...
const writeStruct = (fields: Array<[number, number, any]>): Buffer => {
  return Buffer.concat([
    ...fields.map(([type, id, value]) =>
      Buffer.concat([writeFieldHeader(type, id), writeValue(type, value)]),
    ),
    Buffer.from([0]),
  ])
}
...
const encodeBinaryCallMessage = (
  method: string,
  seqId: number,
  fields: Array<[number, number, any]>,
): Buffer => {
  return Buffer.concat([
    writeI32(0x80010001),
    writeString(method),
...
const readString = (buffer: Buffer, state: { position: number }): string => {
  ...
  const value = buffer
    .subarray(state.position, state.position + length)
    .toString('utf8')
...
readValue = (
  ...
  if (type === 13) {
    ...
    for (let index = 0; index < length; index += 1) {
      entries[String(readValue(buffer, state, keyType))] = readValue(
        buffer,
        state,
        valueType,
      )
...
const postBinary = (
  ...
  body: Buffer,
  token: string,
): Promise<{ body: Buffer; nextToken?: string; statusCode?: number }> => {
...
const buildMessageIdStruct = (
  messageId: MessageBoxV2MessageId,
...
export async function fetchMessagesByIds(
  runtime: any,
  messageBoxId: string,
  messageIds: MessageBoxV2MessageId[],
) {
  const messages: any[] = []
  for (const messageId of messageIds) {
    ...
    if (exception) {
      throw new Error(
        `getMessagesByIds failed: ${exception[2] || JSON.stringify(exception)}`,
      )
    }
