 ...
 */
export function findChatArray(responseRoot: Record<number, unknown> | null | undefined) {
  if (!responseRoot) {
    return null;
  }

  const keys = Object.keys(responseRoot).map(Number).sort((a, b) => a - b);
  ...
  for (const key of keys) {
    ...
    if (key !== 1) {
      lineLog.warn('talk.get_chats.field_mismatch', { actual_field: key, expected_field: 1 });
    }
    return candidate;
  }
 ...
 */
export function mapChatStruct(chat: Record<number, unknown> | null | undefined) {
...
export function mapChatList(chats: unknown): unknown[] {
  return Array.isArray(chats) ? chats.map(chat => mapChatStruct(chat as Record<number, unknown>)).filter(Boolean) : [];
}
 ...
 * @returns Normalized chat list.
 */
export async function fetchChats(runtime: any, syncReason: number, chatMids: string[], withMembers = true): Promise<unknown[]> {
  if (!Array.isArray(chatMids) || chatMids.length === 0) {
    return [];
  }
  if (chatMids.length > GET_CHATS_BATCH_SIZE) {
    const allChats: unknown[] = [];
    for (let index = 0; index < chatMids.length; index += GET_CHATS_BATCH_SIZE) {
  ...
  let result;
  try {
    result = await runtime.sendTalk('getChats', buildGetChatsRequest(chatMids, withMembers, syncReason));
  }
  catch (error: any) {
  ...
  lineLog.warn('talk.get_chats.empty', { keys: response ? Object.keys(response).join(',') : 'null' });
  return [];
}
