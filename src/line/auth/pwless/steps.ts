...
export async function createSession(
  ...
  const fields: ThriftFieldTuple[] = [
    structField(1, [stringField(1, phone), stringField(2, region)]),
  ]
 ...
 */
export async function verifyLoginCertificate(
  ctx: PwlessContext,
  certificate: string,
): Promise<boolean> {
  const fields: ThriftFieldTuple[] = [
...
export async function requestPinCodeVerif(ctx: PwlessContext): Promise<string> {
  const fields: ThriftFieldTuple[] = [
    structField(1, [stringField(1, ctx.sessionId)]),
  ]
 ...
 */
export async function checkPinCodeVerified(
  ctx: PwlessContext,
): Promise<boolean> {
 ...
 */
export async function putExchangeKey(
  ctx: PwlessContext,
  ...
  const fields: ThriftFieldTuple[] = [
    structField(1, [
      ...
      mapField(2, 11, 11, {
        e2eeVersion: '1',
        temporalPublicKey: publicKeyBase64,
      }),
    ]),
  ]
 ...
 */
export async function checkPaakAuthenticated(
  ctx: PwlessContext,
): Promise<boolean> {
