...
export async function cliLogin(argv: string[]): Promise<number> {
  ...
  try {
    ...
    console.log(
      '[Yomi] Starting passwordless login. Enter the PIN on your primary phone within ' +
    ...
    const result = await runPwlessLogin(service, phone, region, {
      onPin: (pin) => {
        ...
        console.log(
          '[Yomi] Enter this PIN in the LINE app on your primary phone, then approve the new device.',
        )
      },
      onWaitingBiometric: () => {
        console.log(
          '[Yomi] Waiting for you to approve the new device on your phone...',
        )
      },
    })
