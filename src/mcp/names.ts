 ...
 */
export async function resolveUserNames(service: LineProtocolService, mids: string[]): Promise<Map<string, string>> {
 ...
 * @returns Map of MID to resolved display name (unresolved MIDs are omitted).
 */
export async function resolveSenderNames(service: LineProtocolService, mids: string[]): Promise<Map<string, string>> {
  const unique = Array.from(new Set(mids.filter(Boolean)));
 ...
 */
export async function resolveGroupNames(service: LineProtocolService, chatIds: string[]): Promise<Map<string, string>> {
 ...
 */
export async function resolveConversationNames(service: LineProtocolService, chatIds: string[]): Promise<Map<string, string | null>> {
  const userIds = chatIds.filter(id => id?.startsWith('u'));
  const groupIds = chatIds.filter(id => id?.startsWith('c') || id?.startsWith('r'));
  const [userNames, groupNames] = await Promise.all([
    userIds.length > 0 ? resolveUserNames(service, userIds) : Promise.resolve(new Map<string, string>()),
    groupIds.length > 0 ? resolveGroupNames(service, groupIds) : Promise.resolve(new Map<string, string>()),
  ]);
